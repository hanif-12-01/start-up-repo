"""Skrip 09: Training Ridge Regression sebagai baseline ML linier."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Training Ridge Regression.")
    parser.add_argument("--data-dir", type=str, default="ML/final")
    parser.add_argument("--output-dir", type=str, default="ML/outputs")
    return parser.parse_args()


def train_ridge(data_dir: Path, output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    train_path = data_dir / "train.csv"
    val_path = data_dir / "validation.csv"
    test_path = data_dir / "test.csv"

    print(f"=== Training Ridge | data_dir={data_dir} output_dir={output_dir} ===")

    if not (train_path.exists() and val_path.exists() and test_path.exists()):
        msg = f"train/validation/test.csv tidak lengkap di {data_dir}."
        print(msg)
        log_to_pipeline(f"[09_train_ridge.py] GAGAL: {msg}")
        return
    ...
    try:
        ...
        joblib.dump(model, output_dir / "ridge_model.pkl")
        ...
        df_predictions.to_csv(output_dir / "ridge_predictions.csv", index=False)
        ...
        df_metrics.to_csv(output_dir / "ridge_metrics.csv", index=False)

        msg = f"Ridge selesai. wMAPE usage: {wmape_u:.2f}% (data_dir={data_dir.name})."
        print(f"=== {msg} ===")
        log_to_pipeline(f"[09_train_ridge.py] SUKSES: {msg}")

    except Exception as e:
        msg = f"Error saat melatih Ridge: {e}"
        print(msg)
        log_to_pipeline(f"[09_train_ridge.py] GAGAL: {msg}")
...
if __name__ == "__main__":
    args = parse_args()
    train_ridge(Path(args.data_dir).resolve(), Path(args.output_dir).resolve())
