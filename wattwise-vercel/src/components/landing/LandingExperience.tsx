...
const story = [
  { number: '01', icon: '🧾', title: 'Catat', text: 'Masukkan tagihan yang sudah Anda punya. kWh, watt alat, dan sensor tidak wajib untuk memulai.' },
  { number: '02', icon: '📊', title: 'Pahami', text: 'Bandingkan biaya per hari dan konteks periode agar perubahan lebih mudah dibaca.' },
  { number: '03', icon: '🔎', title: 'Tentukan prioritas', text: 'Lihat maksimal tiga kandidat yang layak diperiksa, lengkap dengan alasan dan keterbatasannya.' },
  { number: '04', icon: '✅', title: 'Tindak lanjuti', text: 'Gunakan pemeriksaan aman, buat Rencana Hemat, lalu evaluasi hasil pada periode berikutnya.' },
];
...
const personas = [
  ['🏘️', 'Kos & properti kecil', 'Pantau perubahan tagihan area bersama, kamar all-in, pompa air, atau fasilitas operasional.'],
  ['🧺', 'Laundry & UMKM padat energi', 'Hubungkan biaya listrik dengan ritme operasional, pendapatan, dan alat yang perlu ditinjau.'],
  ['🏪', 'Toko, kuliner & usaha berdaya tinggi', 'Susun catatan bulanan, indikasi perubahan, dan rencana pemeriksaan dalam satu alur.'],
];

function BrandMark() {
  return (
    <span className="flex items-center gap-2.5">
      <span aria-hidden="true" className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-600 text-lg text-white shadow-lg shadow-emerald-900/20">⚡</span>
    ...
    <div data-landing-visual className="relative mx-auto w-full max-w-2xl">
      ...
      <div className="overflow-hidden rounded-[2rem] border border-emerald-900/15 bg-[#f8fbf6] shadow-[0_35px_90px_-35px_rgba(6,78,59,0.45)]">
        <div className="flex items-center justify-between border-b border-emerald-900/10 bg-[#edf4e8] px-5 py-3">
          <div className="flex items-center gap-2 text-xs font-bold text-emerald-900"><span aria-hidden="true">⚡</span> Dashboard tindakan</div>
...
export function LandingExperience({ authenticated }: { authenticated: boolean }) {
  ...
  return (
    <div ref={root} className="overflow-hidden bg-[#f7f9f4] text-slate-900">
      ...
      <main>
        ...
        <section aria-label="Prinsip kepercayaan produk" className="border-y border-emerald-900/10 bg-[#edf4e8] px-4 py-5 sm:px-6">
          <div className="mx-auto grid max-w-7xl gap-3 text-center text-xs font-bold text-emerald-950/75 sm:grid-cols-2 lg:grid-cols-4">
            <p>✍️ Input manual yang sederhana</p><p>📡 Tidak memerlukan sensor</p><p>🧭 Estimasi dijelaskan transparan</p><p>🔐 Data usaha terisolasi</p>
          </div>
        </section>
        ...
        <section id="produk" className="scroll-mt-24 px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {[['📈', 'Biaya berubah', 'Bandingkan periode dan biaya harian agar angka nominal tidak menyesatkan.'], ['❓', 'Penyebab belum jelas', 'Kumpulkan konteks usaha dan susun kandidat, tanpa menyatakan penyebab pasti.'], ['🧰', 'Tidak tahu harus mengecek apa', 'Ikuti pemeriksaan observasional yang aman dan mudah dipahami.'], ['🔁', 'Sulit melihat hasil', 'Bandingkan periode berikutnya dengan baseline dan catat konteks perubahannya.']].map(([icon, title, text]) => <article key={title} data-reveal className="rounded-3xl border border-emerald-900/10 bg-white p-6"><span className="text-2xl" aria-hidden="true">{icon}</span><h3 className="mt-4 font-extrabold text-emerald-950">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{text}</p></article>)}
            </div>
          </div>
        </section>
        ...
        <section id="cara-kerja" className="scroll-mt-24 bg-emerald-950 px-4 py-20 text-white sm:px-6 lg:px-8 lg:py-28">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-12 grid gap-4 lg:grid-cols-4">
              {story.map((item) => <article key={item.number} data-story-card className="rounded-3xl border border-emerald-300/15 bg-white/5 p-6 backdrop-blur"><div className="flex items-center justify-between"><span className="text-2xl" aria-hidden="true">{item.icon}</span><span className="text-xs font-black text-emerald-300">{item.number}</span></div><h3 className="mt-6 text-xl font-black">{item.title}</h3><p className="mt-3 text-sm leading-6 text-emerald-50/70">{item.text}</p></article>)}
            </div>
          </div>
        </section>
        ...
        <section id="untuk-siapa" className="scroll-mt-24 bg-[#edf4e8] px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div className="mx-auto max-w-7xl"><div data-reveal className="max-w-3xl"><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-emerald-700">Untuk siapa</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em] text-emerald-950 sm:text-4xl">Untuk pengelola yang dekat dengan operasional.</h2></div><div className="mt-10 grid gap-5 lg:grid-cols-3">{personas.map(([icon, title, text]) => <article key={title} data-reveal className="rounded-3xl border border-emerald-900/10 bg-[#f9fbf7] p-7"><span className="text-3xl" aria-hidden="true">{icon}</span><h3 className="mt-5 text-xl font-black text-emerald-950">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{text}</p></article>)}</div></div>
        </section>
        ...
        <section id="transparansi" className="scroll-mt-24 px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div data-reveal className="mx-auto grid max-w-7xl gap-8 rounded-[2rem] border border-emerald-900/10 bg-white p-7 shadow-[0_30px_80px_-45px_rgba(6,78,59,0.4)] md:p-10 lg:grid-cols-[0.8fr_1.2fr]">
            <div><span className="text-3xl" aria-hidden="true">🧭</span><p className="mt-4 text-xs font-extrabold uppercase tracking-[0.2em] text-emerald-700">Transparansi sejak awal</p><h2 className="mt-3 text-3xl font-black tracking-[-0.04em] text-emerald-950">Anda tetap memegang keputusan.</h2></div>
            <ul className="grid gap-3 text-sm leading-6 text-slate-600 sm:grid-cols-2">
              {['Hasil bergantung pada data yang Anda masukkan.', 'Estimasi bukan tagihan resmi atau audit energi.', 'Kandidat bukan pernyataan penyebab atau alat rusak.', 'Pemeriksaan alat tetap dilakukan secara manual dan aman.', 'Tidak ada pengukuran real-time tanpa sensor.', 'WattWise bukan aplikasi resmi dan tidak berafiliasi dengan PLN.'].map((item) => <li key={item} className="rounded-2xl bg-emerald-50/70 p-4"><span className="mr-2 text-emerald-600" aria-hidden="true">✓</span>{item}</li>)}
            </ul>
          </div>
        </section>
