import { Plus, PlugZap, Puzzle } from 'lucide-react';
import { decimal } from '@/lib/format';
import { BusinessSelector, EmptyState, fieldClass, primaryButton, secondaryButton, SoftCard, WorkspaceHeader, WorkspacePage } from '@/components/product/WorkspaceUI';
    ...
    <WorkspacePage>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.76fr_1.24fr]">
        <div className="space-y-6">
          <SoftCard>
            <div className="flex items-start gap-3"><Puzzle className="mt-0.5 h-6 w-6 shrink-0 text-[var(--primary)]" aria-hidden="true"/><div><h2 className="font-extrabold text-[var(--foreground)]">Template cepat</h2><p className="mt-1 text-sm leading-6 text-[var(--muted)]">Preset mengikuti segmen {data.business.segment}. Setelah diterapkan, sesuaikan dengan kondisi usaha yang sebenarnya.</p></div></div>
            <form action={applyTemplateAction} className="mt-5"><input type="hidden" name="businessId" value={data.business.id} /><button className={`${secondaryButton} w-full`}>Gunakan Template Segmen</button></form>
          </SoftCard>
          <SoftCard>
            <h2 className="flex items-center gap-2 text-lg font-extrabold text-[var(--foreground)]"><Plus className="h-5 w-5 text-[var(--primary)]" aria-hidden="true"/>Tambah peralatan</h2>
            <form action={addApplianceAction} className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="sm:col-span-2"><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Nama alat</span><input name="name" required minLength={2} placeholder="Contoh: Pompa air utama" className={fieldClass} /></label>
              <label className="sm:col-span-2"><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Kategori</span><input name="category" required placeholder="Pendingin, pencahayaan, sistem air..." className={fieldClass} /></label>
              <label><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Daya (W) · opsional</span><input name="powerWatts" type="number" min="0" placeholder="Tidak tahu boleh kosong" className={fieldClass} /></label>
              <label><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Jam/hari · opsional</span><input name="dailyHours" type="number" min="0" max="24" step="0.5" placeholder="Contoh: 8" className={fieldClass} /></label>
              <label><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Jumlah</span><input name="quantity" type="number" min="1" defaultValue="1" className={fieldClass} /></label>
              <label><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Hari operasi/bulan</span><input name="operatingDays" type="number" min="1" max="31" defaultValue="30" className={fieldClass} /></label>
              <label className="sm:col-span-2"><span className="mb-1 block text-xs font-bold text-[var(--foreground)]">Catatan opsional</span><textarea name="notes" rows={2} className={fieldClass} /></label>
              <button className={`${primaryButton} sm:col-span-2`}>Simpan Peralatan</button>
            </form>
          </SoftCard>
        </div>
