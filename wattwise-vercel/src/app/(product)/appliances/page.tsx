      ...
      <div className="grid gap-6 xl:grid-cols-[0.76fr_1.24fr]">
        <div className="space-y-6">
          ...
          <SoftCard>
            ...
            <form action={addApplianceAction} className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="sm:col-span-2"><span className="mb-1 block text-xs font-bold">Catatan opsional</span><textarea name="notes" rows={2} className="w-full rounded-xl border border-emerald-900/15 bg-[#fbfcfa] px-3 py-2.5 text-sm" /></label>
        ...
        <SoftCard>
          ...
          {data.appliances.length === 0 ? <div className="mt-6"><EmptyState icon="🔌" title="Belum ada peralatan" description="Gunakan template atau tambah alat yang relevan. Watt dan jam pakai boleh dikosongkan jika tidak diketahui." /></div> : (
            <div className="mt-6 space-y-3">{data.appliances.map((item) => { const estimate = estimateMonthlyKwh(item); return <article key={item.id} className={`rounded-2xl border border-emerald-900/10 p-4 ${item.isActive ? 'bg-white' : 'bg-slate-50 opacity-65'}`}><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-extrabold text-emerald-950">{item.name}</h3><p className="mt-1 text-xs text-slate-500">{item.category} · {item.quantity} unit · {item.operatingDays} hari</p><p className="mt-2 text-sm font-extrabold text-emerald-700">{estimate === null ? 'Belum cukup data' : `${decimal.format(estimate)} kWh/bulan`}</p></div><form action={toggleApplianceAction}><input type="hidden" name="businessId" value={data.business.id}/><input type="hidden" name="applianceId" value={item.id}/><input type="hidden" name="active" value={String(!item.isActive)}/><button className="rounded-lg border border-emerald-900/15 px-3 py-2 text-xs font-bold">{item.isActive ? 'Nonaktifkan' : 'Aktifkan'}</button></form></div><details className="mt-4 border-t border-emerald-900/10 pt-4"><summary className="cursor-pointer text-sm font-bold text-emerald-800">Edit detail</summary><form action={updateApplianceAction} className="mt-4 grid gap-3 sm:grid-cols-2"><input type="hidden" name="businessId" value={data.business.id}/><input type="hidden" name="applianceId" value={item.id}/><input name="name" defaultValue={item.name} required className="rounded-xl border p-2.5"/><input name="category" defaultValue={item.category} required className="rounded-xl border p-2.5"/><input name="powerWatts" type="number" min="0" defaultValue={item.powerWatts ?? ''} className="rounded-xl border p-2.5"/><input name="dailyHours" type="number" min="0" max="24" step="0.25" defaultValue={item.dailyHours ?? ''} className="rounded-xl border p-2.5"/><input name="quantity" type="number" min="1" defaultValue={item.quantity} className="rounded-xl border p-2.5"/><input name="operatingDays" type="number" min="1" max="31" defaultValue={item.operatingDays} className="rounded-xl border p-2.5"/><textarea name="notes" defaultValue={item.notes ?? ''} className="rounded-xl border p-2.5 sm:col-span-2"/><div className="flex gap-2 sm:col-span-2"><button className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white">Simpan Perubahan</button><button formAction={deleteApplianceAction} className="rounded-xl border border-rose-300 px-4 py-2 text-sm font-bold text-rose-700">Hapus</button></div></form></details></article>; })}</div>
          )}
